...
const TracksPage = () => {
  ...
  const colorMap: Record<string, { bg: string, text: string, light: string, border: string, glow: string }> = {
    purple: { 
          ...
          >
            {tracks.map((track, index) => (
              <motion.div 
                ...
                className={`glass-effect backdrop-blur-md border ${colorMap[track.color].border} rounded-xl overflow-hidden hover:${colorMap[track.color].glow} transition-all duration-300`}
              >
                <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="col-span-1">
                    <div className={`w-16 h-16 rounded-full ${colorMap[track.color].light} flex items-center justify-center mb-6 animate-pulse-light`}>
                    ...
                    <Link 
                      to="/register" 
                      className={`inline-block ${colorMap[track.color].bg} text-white font-semibold rounded-lg py-2 px-4 transition-all hover:opacity-90 hover:-translate-y-1 duration-300`}
                    >
                      Select this Track
                    </Link>
                  </div>
                  ...
                  <div className="lg:col-span-2 bg-hackathon-navy/50 backdrop-blur-sm rounded-lg p-6">
                    <h4 className="text-lg font-semibold mb-4 text-white">Problem Statements</h4>
                    <ul className="space-y-4">
                      {track.problems.map((problem, i) => (
                        <li key={i} className="flex items-start group">
                          <div className={`w-6 h-6 rounded-full ${colorMap[track.color].bg} flex items-center justify-center mr-3 mt-0.5 flex-shrink-0 group-hover:scale-110 transition-transform duration-300`}>
                            <span className="text-white text-sm font-bold">{i+1}</span>
                          </div>
                          <p className="text-gray-200 group-hover:text-white transition-colors duration-300">{problem}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
